...
(function() {
   maptune.saveCollection = function(){
	 layer = maptune.jsapi.getLayer("*","Collection");
	 layer = layer.data;
	 var x = new Theme(layer);
	 var szJson = x.getPrettyString();
	try	{
		maptune.confirmMessage("Please confirm download",
			function () {
   ...
   maptune.loadCollection = function(){
 ...
 */
	...
	Theme.prototype.parse = function(szThemeDef){
		...
		try	{
			...
			maptune.parentApi.error("Code: "+e);
		}
	};
...
maptune.jsapi.import(
	{ "type": "Collection",
	  "name": "collection",
	  "title": "La mia mappa",
	  "description": "<div id='infodiv' style='margin-top:-10px;color:#444;'>Contiene i punti collezionati<br><label class='btn btn-default' title='scarica GeoJson' onclick='maptune.saveCollection();'>download GeoJson</label><div>",
