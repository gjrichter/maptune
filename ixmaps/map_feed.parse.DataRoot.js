 ...
 */
	ixmaps.feed.parse.DataRoot = function(data,opt){
		...
		if ( opt.format == "json" ) {
			...
			// ----------------------
			var layerset = new ixmaps.feed.LayerSet(szItem);
			...
			for ( i=0; i<itemA.length; i++ ){
				...
				// it was a try, to scroll only the text, must add ixmaps.touchScroll("..."); to get it working
