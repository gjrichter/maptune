 ...
 */
	ixmaps.feed.parse.GeoRSS = function(data,opt){
		...
		if ( opt.format == "xml" ) {
			...
			if ( $(data).find('rss').length ){
				return ixmaps.feed.parse.GeoRSS_RSS(data,opt);
			}else
			if ( $(data).find('feed').length ){
				return ixmaps.feed.parse.GeoRSS_ATOM(data,opt);
			}
		}
		return null;
	};

	ixmaps.feed.parse.GeoRSS_RSS = function(data,opt){
	...
	ixmaps.feed.parse.GeoRSS_ATOM = function(data,opt){
