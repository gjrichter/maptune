 ...
 * @return A new MapUp
 */
ixmaps.MapUp = MapUp = function(map,itemDiv,legendDiv) {
...
MapUp.prototype.addLayer = function(szSourceName,source) {
	...
	for ( var i=0; i<layerCount; i++ ){
		...
		//	
		if ( ixmaps.szMasterSource && ixmaps.szLinkLayer ){

			if ( layer.properties.name != ixmaps.szLinkLayer ){
				layer.properties.name = ixmaps.szLinkLayer;
			}
			...
			if ( layer.features || (layer.layers.length == 1) ){
				szSourceName = ixmaps.szMasterSource;
			}else{
				szSourceName = ixmaps.szMasterSource;
				this.removeLayer(ixmaps.szLinkLayer);
			}
			ixmaps.szMasterSource = null;
			ixmaps.szLinkLayer = null;
		}
		...
		if ( layer.features ){
			...
			layer.legend = "expanded";
			if ( !this.szItemFilter && !ixmaps.jsapi.fShowExpanded ){
	...
	if ( nDynLayerNotVisible && (nDynLayerNotVisible == (tempLayerA.length-1)) ){
		ixmaps.confirmMessage("Layer not visible at this scale!<br>Zoom in to see the layer?",function(){
			ixmaps.jsapi.changeZoomToScale(nDynLayerScale);
		});
	}

	// finally add the parsed layer
	this.layer = tempLayerA.concat(this.layer);
	_TRACE("addLayer: done");
...
MapUp.prototype.isLayerInMapScale = function(layer) {
	...
	if ( !layer.data.fOverrideScaleDependency ){
...
MapUp.prototype.setExtent = function(szMode,szLayer) {
	...
	if ( szMode.match(/tolayer/) && szLayer && this.data ){
		...
		this.actualMapCenter = _map_getCenter();
		this.actualMapZoom = _map_getZoom();
...
MapUp.prototype.changeZoom = function(nDelta) {
	...
	_map_setZoom(this.actualMapZoom+nDelta);
	this.actualMapZoom = _map_getZoom();
};
/**
...
MapUp.prototype.redrawLayer = function() {
	...
	for ( var i=0; i<this.layer.length; i++ ){
		...
		if ( ixmaps.jsapi.szLayerType &&
			 !eval("type.match(/"+ixmaps.jsapi.szLayerType+"/)") ){
...
MapUp.prototype.clearMap = function() {
	for ( var i=0; i<this.layer.length; i++ ){
		if ( this.layer[i].source ){
			ixmaps.jsapi.setDataCheckBox(this.layer[i].source,false);
		}
		this.clearOneLayer(this.layer[i]);
	}
...
MapUp.prototype.drawSidebar = function() {
	...
	if ( !nLayer ){
		...
		var szEmpty = (ixmaps.jsapi.mapParam && ixmaps.jsapi.mapParam.noItemText)?ixmaps.jsapi.mapParam.noItemText:"(empty)";
	...
	if ( !this.fUseListStateButtons && ($("#sidebar-footer")[0]) ){
		...
		szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(null,\"none\");\'><img src=\"resources/ui/listview.jpg\" height=\"32\" style='opacity:"+(this.szListContent=="none"?0.5:0.2)+";border:"+(this.szListContent=="none"?"#666 solid 1px":"0")+";'></a>";
		szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(null,\"info\");\'><img src=\"resources/ui/articelview.jpg\" height=\"32\" style='opacity:"+(this.szListContent=="info"?0.5:0.2)+";border:"+(this.szListContent=="info"?"#666 solid 1px":"0")+";'></a>";
		szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(null,\"gallery\");\'><img src=\"resources/ui/galleryview.jpg\" height=\"32\" style='opacity:"+(this.szListContent=="gallery"?0.5:0.2)+";border:"+(this.szListContent=="gallery"?"#666 solid 1px":"0")+";'></a>";
		szHTML += "<a class=\"listitemcontrol\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(null,\"hideall\");\'><img src=\"resources/ui/collapse.jpg\" height=\"32\" style='opacity:"+(this.szListContent=="gallery"?0.5:0.2)+";border:"+(this.szListContent=="gallery"?"#666 solid 1px":"0")+";'></a>";
		szHTML += "<a href=\"javascript:javascript:ixmaps.jsapi.setMapContent(null,'show');\"><img src='resources/ui/checkbox_checked.gif' height='22' style=\"opacity:0.2;float:right;margin-right:11px;margin-top:5px\" /></a>";
		szHTML += "<a href=\"javascript:javascript:ixmaps.jsapi.setMapContent(null,'hide');\"><img src='resources/ui/box.gif' height='22' style=\"opacity:0.2;float:right;margin-right:11px;margin-top:5px\" /></a>";
		szHTML += "</div>";
 ...
 * @return true or false
 */
ixmaps.MapUp.prototype.redraw = function() {
//	this.clearLayer();
...
MapUp.prototype.synchronize = function() {
	...
	// --------------------------------
	if ( this.lastZoom && (this.lastZoom > this.actualMapZoom) ){
		for ( var i=0; i<this.layer.length; i++ ){
...
MapUp.prototype.makeItemList = function(layerObj) {
	...
	if ( ixmaps.jsapi.szLayerType ){
		...
		if ( !eval("type.match(/"+ixmaps.jsapi.szLayerType+"/)") ){
			return 0;
		}
	}
	if ( !this.itemList ) {
	...
	if ( (layer.properties.legendstyle && layer.properties.legendstyle.match(/NOLEGEND/)) ){
		return 0;
	}
    var listItem = null;
		...
		if ( (this.lastSource != layer.source) ){
			...
			var szHTML = "";
				if ( layer.sourcelegend == "collapsed" ){
					szHTML += "<a class=\"title collapsed\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.source+"\",\"showall\");\'>";
					szHTML += szName;
					...
					szHTML += "<a class=\"title expanded\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.source+"\",\"hideall\");\'>";
					szHTML += szName;
					szHTML += "</a>";
					//szHTML += "<a href=\"javascript:ixmaps.jsapi.removeData('"+layer.source+"')\"><img src='resources/ui/delete.png' height='22' style=\"opacity:0.3;float:right;margin-right:11px;margin-top:5px\" /></a>";
	...
	if ( 1 ){
		...
		if ( (layer.properties.name != layer.source)){
			...
			if ( layer.link ){
				...
				listItem.innerHTML += "<a class=\"listitemname\" href=\'javascript:ixmaps.szMasterSource=\""+layer.source+"\";ixmaps.szLinkLayer=\""+layer.properties.name+"\";ixmaps.jsapi.addData(\""+szLayer+"\",\"\",\""+szLayerSource+"\");\'><img src='resources/ui/box.gif' height='22' style=\"vertical-align:-10px\" /></a>" ;
			}		
			else
			// layer loaded, add button to show/hide
			if ( markers.length){
				if ( layer.legend == "collapsed" ){
					listItem.innerHTML +=  "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"show\");\'><img src='resources/ui/plusbox2.gif' height='22' style=\"vertical-align:-10px\" /></a>" ;
				}else{
					listItem.innerHTML +=  "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"hide\");\'><img src='resources/ui/minusbox2.gif' height='22' style=\"vertical-align:-10px\" /></a>" ;
				}
				if ( ixmaps.jsapi.fCheckLayer || (layer.properties.legendstyle && layer.properties.legendstyle.match(/CHECKSUBLAYER/))){
					if ( layer.properties.visibility == "0" ){
						listItem.innerHTML += "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setMapContent(\""+layer.properties.name+"\",\"show\");\'><img src='resources/ui/box.gif' height='22' style=\"float:right;margin-right:10px;vertical-align:-5px\" /></a>" ;
					}else{
						listItem.innerHTML += "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setMapContent(\""+layer.properties.name+"\",\"hide\");\'><img src='resources/ui/checkbox_checked.gif' height='22' style=\"float:right;margin-right:10px;vertical-align:-5px\" /></a>" ;
					}
				}
			}
		}
		...
		if ( layer.link ){
			...
			// listItem.innerHTML += "<a class=\"listitemcontrol\" title=\"zoom fin che visibile\" href=\"javascript:ixmaps.jsapi.changeZoomToScale("+layerObj.upper+");\" style=\"color:#bbbbbb\">(not loaded)</a>";
		}else
		...
		if ( !markers.length ){
			...
			if ( 0 && layer.properties.name && layer.properties.name.length && layer.layers && (layer.layers.length > 1) ){
				...
				//
				if ( layer.legend == "collapsed" ){
					listItem.innerHTML +=  "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"show\");\'><img src='resources/ui/plusbox2.gif' height='22' style=\"margin-left:0px;margin-right:5px;\" /></a>" ;
				}else{
					listItem.innerHTML +=  "<a class=\"listitemname\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"hide\");\'><img src='resources/ui/minusbox2.gif' width='22' style=\"margin-left:0px;margin-right:5px;\" /></a>" ;
				}
				if ( layer.properties.name != layer.source ){
		...
		if ( layerObj.upper && (layerObj.upper < this.actualMapScale) && !layer.fOverrideScaleDependency){
			...
			if ( !(layer.legend == "collapsed") ){
				listItem.innerHTML += "<br/><span class=\"listitemcontrolindent\"></span> ";
				listItem.innerHTML += "<a class=\"listitemcontrol\" title=\"zoom fin che visibile\" href=\"javascript:ixmaps.jsapi.changeZoomToScale("+layerObj.upper+");\" style=\"color:#bbbbbb\">(non visibile a questo livello zoom)</a>";
				listItem.innerHTML += "<a class=\"listitemcontrol\" title=\"zoom fin che visibile\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"overridescaledependency\");\' style=\"color:#bbbbbb\"> show</a>";
			}
		}else{
			// layer visible
			// ---------------------------------------
			...
			if ( (layer.fUseListStateButtons || (layer.parent && layer.parent.fUseListStateButtons )) && (layer.legend !== "collapsed") ){
				var szHTML = "";
				szHTML += "<div class=\"listitemcontrolgroup\" >";
				szHTML += "<a class=\"listitemcontrol\" style=\"float:left\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"none\");\'><img src=\"resources/ui/listview.jpg\" height=\"32\" style='opacity:"+(!layer.fShowInfoInList&&!layer.properties.fGallery?0.2:0.2)+";border:"+(!layer.fShowInfoInList&&!layer.properties.fGallery?"#666 solid 1px":"0")+";'></a>";
				szHTML += "<a class=\"listitemcontrol\" style=\"float:left\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"info\");\'><img src=\"resources/ui/articelview.jpg\" height=\"32\" style='opacity:"+(layer.fShowInfoInList?0.2:0.2)+";border:"+(layer.fShowInfoInList?"#666 solid 1px":"0")+";'></a>";
				szHTML += "<a class=\"listitemcontrol\" style=\"float:left\" title=\"cambia contenuto del elenco\" href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"gallery\");\'><img src=\"resources/ui/galleryview.jpg\" height=\"32\" style='opacity:"+(layer.properties.fGallery?0.2:0.2)+";border:"+(layer.properties.fGallery?"#666 solid 1px":"0")+";'></a>";
...
MapUp.prototype.makeItemListItems = function(layerObj,defaultIcon,markers) {
	...
	var maxItems = (ixmaps.jsapi.mapParam && ixmaps.jsapi.mapParam.maxitems)?ixmaps.jsapi.mapParam.maxitems:250;
	var nItems = 0;
	var szLastDate = "";
    var listItem = null;
	...
	if ( layer.fShowAll ){
		...
		$("<a href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"clip\");\' >&nbsp;nascondi elementi fuori quadro</a>").appendTo(listItem); 
	...
	for(var i=0; i < markers.length && (nItems < maxItems); i++) {
		...
		if ( markers[i].inLegend && !markers[i].fSuppressInfo ){
			...
			try	{
				szTitle = ixmaps.jsapi.onInfoWindowTitle(szTitle,markers[i]);
			}
			catch (e){}
			...
			if ( layer.fShowInfoInList && markers[i].properties.description && !markers[i].fSuppressInfo ){
				...
				if ( 0 && markers[i].properties.data.SITO ){
					szRowContent += " - "+"<a href=\'javascript:ixmaps.jsapi.openlink(\""+markers[i].properties.data.SITO+"\");\' >"+markers[i].properties.data.SITO+ "</a>" ; 
				}
				...
				if ( 0 && markers[i].properties.data.LNK_2 ){
					...
					szRowContent += "<span class=\"noprint\"> - "+"<a class=\"noprint\" title=\'ascolti audio\' href=\'javascript:ixmaps.jsapi.playaudio(\""+markers[i].properties.data.LNK_2+"\");\' >" + szMp3A[szMp3A.length-1] + "</a></span>" ; 
				}
	...
	if ( nItems >= maxItems ){
		...
		if ( nItems == 0 ){
			$("<div style='text-align:center'>( nessun elemento )</div><br>").appendTo(listItem); 
		}
		...
		if ( nMarkerOutside ){
			$("<a href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"piu\");\' >&nbsp;+ "+(nMarkerOutside)+" elementi fuori quadro<img src='resources/ui/zoomto.png' style='margin-left:10px;vertical-align:-2px;height:14px'/></a>").appendTo(listItem); 
		}
		listTable.appendChild(listItem);
	}
	return nItems;
...
_mapup_collect_item = function(szSourceName,szLayerName,nItem){
	...
	if ( layer ){
		...
		// inform user
		ixmaps.infoMessage("added to: '"+target.title+"'");

		// update local storage
		var x = new Theme(target.data);
		var szJson = x.getPrettyString();
		ixmaps.storeObject("maptune-collection", szJson);
	}	
};
/*
 ...
 * @param nItem JSON the index of the item to copy 
 */
_mapup_get_collected_items = function(){

	// get collection from local storage
	var collection = ixmaps.getStoredObject("maptune-collection");
...
MapUp.prototype.makeItemListGallery = function(layerObj,defaultIcon,markers) {
	...
	for(var i=0; i < markers.length && (nItems < maxItems); i++) {
		...
		if ( markers[i].inLegend && !markers[i].fSuppressInfo ){
			...
			if ( this.timeline && layer.duration && (szDate != szLastDate) ){
				var dateItem = document.createElement("p");
				dateItem.setAttribute("style","font-family:arial;font-size:0.8em;color:#53637D;margin-top:5px;margin-bottom:2px;border-bottom:solid 1px #ddd");
                var szPiu = "";
                if ( szDate == _mapup_explDate ){
					szPiu = " <a href=\"javascript:_mapup_open_sidebar_datesection('')\" style=\"font-size:0.8em;\">";
				}else{
            ...
            var szListImage = "";
			if ( (!layer.duration || (markers.length < 500) || (szDate == _mapup_explDate)) && markers[i].properties.description.match(/\<img/i) ){
				var match = null;
				if (  markers[i].properties.description.match(/thumbnail/i) ){
					match = markers[i].properties.description.match(/<img[^>]*src=(?:\"|\')([^\"]*)\"[^>]*>.*/i);
				}else{
					match = markers[i].properties.description.match(/<img.+?src=[\"\'](.+?)[\"\'].*?>/i);
					//match = markers[i].properties.description.match(/src=(?:\"|\')?([^>]*[^/].(?:jpg|jpeg|bmp|gif|png)[^\"|\'>]*)(?:\"|\')?/i);
				}
				var szImg = (match && (match.length>1))?(match[1]):"";
				szListImage = "<a href=\"javascript:void()\"><img class='gallery-image' src='"+szImg+"' /></a>";
			}else{
	...
	if ( nItems >= maxItems ){
		...
		listItem.setAttribute("class","listitem");
		//$("<a href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"piu\");\' >&nbsp;&nbsp;- tutti "+(markers.length-nItems)+" elementi fuori quadro</a>").appendTo(listItem); 
		$("<a href=\'javascript:ixmaps.jsapi.zoomToLayer(\""+layer.properties.name+"\");\' >&nbsp;&nbsp;- tutti "+(markers.length-nItems)+" elementi fuori quadro</a>").appendTo(listItem); 
		this.itemList.appendChild(listItem);
	}
	else if ( nItems < markers.length ){
		listItem = document.createElement("div");
		listItem.setAttribute("class","listitem");
		//$("<a href=\'javascript:ixmaps.jsapi.setListContent(\""+layer.properties.name+"\",\"piu\");\' >&nbsp;&nbsp;- tutti "+(markers.length-nItems)+" elementi fuori quadro</a>").appendTo(listItem); 
		$("<a href=\'javascript:ixmaps.jsapi.zoomToLayer(\""+layer.properties.name+"\");\' ><br>&nbsp;&nbsp;"+(markers.length-nItems)+" elementi fuori quadro</a>").appendTo(listItem); 
		this.itemList.appendChild(listItem);
	}
};
...
/**
 * lookup szLayerName in ixmaps.jsapi.mapParam object, and return the parameter of the layer
 * @param szLayerName the name of the layer
 * @return an object with layer parameter or null
 */
function _mapup_getLayerParam(szLayerName){
	var layerP = null;
	if ( ixmaps.jsapi.mapParam && ixmaps.jsapi.mapParam.layers ){
...
function _mapup_getIcon(map,defaultIcon,layer,markers,i,szFlag) {	
		...
		if ( typeof(GIcon) == 'undefined' ){
			return new Object({image:ixmaps.jsapi.getResourceUrl(markers[i].properties.icon)});
		}
		...
		if (1){
		try	{
			var userIcon = ixmaps.jsapi.onGetIcon(icon,markers[i],i);
			if ( userIcon )	{
				return userIcon;
			}
		}
		...
		if ( layerP && (layerP.markerType == "image") && markers[i].properties.data && markers[i].properties.data.IMMAGINE ){
			...
			icon.image = ixmaps.jsapi.getResourceUrl(layerP.thumbnailRoot+markers[i].properties.data.IMMAGINE);
			icon.printImage = ixmaps.jsapi.getResourceUrl(layerP.thumbnailRoot+markers[i].properties.data.IMMAGINE);
			icon.shadow = ixmaps.jsapi.getResourceUrl("resources/icons/image_shadow.png");
		...
		if ( layerP && (layerP.markerType == "numeric") ){
			var myIcon = ixmaps.jsapi.getResourceUrl("resources/icons/Google Maps Icons/iconr"+ (i+1) +".png");
						...
						if ( eval("data.match(/"+a+"/i)") ) {
							if ( ((szFlag && (szFlag == 'small')) || (layerP.iconType == 'small') ) ){
								if ( layerP.categoryList[a].smallImage ){
									...
									myIcon = ixmaps.jsapi.getResourceUrl(_mapup_szDefaultSmallMarker);
									icon.iconSize = new GSize(15, 15);
									icon.iconAnchor = new GPoint(7, 7);
									icon.infoWindowAnchor = new GPoint(15,6);
									break;
		...
		if (  markers[i].properties.icon || markers[i].properties.smallicon ){
			...
			if ( layer.fShowInfoInList && markers[i].properties.icon){
				fSmallIcon = false;
			}
			...
			if ( markers[i].properties.smallicon && fSmallIcon ){
				...
				icon.image = ixmaps.jsapi.getResourceUrl(markers[i].properties.smallicon);
				icon.printImage = ixmaps.jsapi.getResourceUrl(markers[i].properties.smallicon);
				icon.iconSize = new GSize(15*nScale, 15*nScale);
				icon.iconAnchor = new GPoint(7*nScale, 7*nScale);
				icon.legendimage = ixmaps.jsapi.getResourceUrl(markers[i].properties.icon);

			}else{
				var nScale = markers[i].properties.iconscale? markers[i].properties.iconscale:1; 
				icon.image = ixmaps.jsapi.getResourceUrl(markers[i].properties.icon);
				icon.printImage = ixmaps.jsapi.getResourceUrl(markers[i].properties.icon);
				icon.iconSize = new GSize(32*nScale, 37*nScale);
				//icon.iconSize = new GSize(24, 24);
				var szShadowA = markers[i].properties.icon.split(".");
				icon.shadow = ixmaps.jsapi.getResourceUrl(String("./"+szShadowA[1]+"_shadow."+szShadowA[2]));
		...
		if ( szFlag && (szFlag == 'small') ){
			icon.image = ixmaps.jsapi.getResourceUrl(_mapup_szDefaultSmallMarker);
			icon.printImage = ixmaps.jsapi.getResourceUrl(_mapup_szDefaultSmallMarker);
			icon.iconSize = new GSize(15, 15);
			icon.iconAnchor = new GPoint(7, 7);
			icon.infoWindowAnchor = new GPoint(15,6);
		}
...
MapUp.prototype.makeLayer = function(map,layerObj,defaultIcon) {	
	...
	if (!layer.duration && markers[0].properties.utime){
		...
		if ( !this.timeline ){
			...
			this.setTimeScope("today");
			if ( markers.length < 150 ){
				this.setTimeScope("all");
			}
			if ( _mapup_getLayerParam(layerObj.name) ){
				this.setTimeScope(_mapup_getLayerParam(layerObj.name).initialTimeScope);
			}
		}
	}
	...
	if (1){
		...
		if ( layerObj.nMarkerCount && (
				( (__nCount < __nLargeIconMaxCount) &&  (layerObj.nMarkerCount > __nLargeIconMaxCount) ) ||
				( (__nCount > __nLargeIconMaxCount) &&  (layerObj.nMarkerCount < __nLargeIconMaxCount) ) 
				)
			){
			this.clearOneLayer(layerObj);	
	...
	for( var i=0; i < markers.length; i++ ) {
		...
		layerObj.fNoSmall = (layerObj.nMarkerCount < __nLargeIconMaxCount)?true:false;
		...
		if ( (__timer_getMS() > 250) || (this.nMarker > this.maxFlushMarker) ){
			ixmaps.xmapUp = this;
			setTimeout("ixmaps.xmapUp.drawLayer()",10);
			return;
		}
	}
	if (this.markerClusterer !== null) {
...
MapUp.prototype.makeOverlayObjects = function(map,layer,markers,i,g,defaultIcon,szFlag) {	
	...
	try	{
		__szTooltip = ixmaps.jsapi.onMarkerTooltip(__szTooltip,markers[i]);
	}
	catch (e){}
	...
	if ( __type == "Polygon" ){
		...
		var polygon = new GPolygon(	polyPoints,
									markers[i].properties.style.lineStyle.color,
									markers[i].properties.style.lineStyle.width*10,1,
									markers[i].properties.style.fillStyle.color,
									markers[i].properties.style.fillStyle.opacity?markers[i].properties.style.fillStyle.opacity:0.3);
...
//				polygon.myPosition = new GLatLng(geometry.coordinates[0][1], geometry.coordinates[0][0]);
	...
	// ---------------
		...
		var polyline = new GPolyline(polyPoints,markers[i].properties.style.lineStyle.color,Number(markers[i].properties.style.lineStyle.width),markers[i].properties.style.lineStyle.opacity||1.0);
...
function __doGetPolygonCenterGLatLng(ptList){
	...
	for ( var a in ptList ){
		ptCenter.lat += ptList[a].lat || ptList[a].lat();
		ptCenter.lng += ptList[a].lng || ptList[a].lng();
	}
			...
			if ( markers[i].geometry ){
				switch (markers[i].geometry.type){
					...
					default:
						...
						if ( icon && markers[i].properties.icon && (icon.image == ixmaps.jsapi.getResourceUrl(markers[i].properties.icon)) ){
 ...
 * look in ixmaps.jsapi.mapParam for dictionary
 * @param szItemText the original legend item text
 */
MapUp.prototype.translateLegendItem = function(szItemText) {	
	if ( ixmaps.jsapi.mapParam.legend && ixmaps.jsapi.mapParam.legend.dictionary ) {
		var dict = ixmaps.jsapi.mapParam.legend.dictionary;
		for ( var a in dict ){
...
MapUp.prototype.highlightSidebarItem = function(map,marker) {
	...
	if ( marker.listItem ){
		...
		var item = marker.listItem;
		var bgItem = item.firstChild.nextSibling;
		try{
			bgItem = item;
		}
		catch (e){}
        ...
        /***
    	this.oldHighLightItem = bgItem;
		this.oldHighLightItemBg = bgItem.style.background;
		this.oldHighLightMarker = marker;
		bgItem.style.background = _mapup_szItemHighlight;
		bgItem.style.border="1";
	    **/	
        this.oldHighLightItem = bgItem;
        bgItem.setAttribute("class","listitem listitemselected");
		// calcolate item offset and if outside, scroll sidebar
		this.scrollToSidebarItem(map,item);
	}
};
...
function _mapup_createMarkerClickHandlerGoto(map, marker, layer, info, szMode, i ) {
	return function() {
		...
		try {
			...
			ixmaps.jsapi.panTo(coordinates[1],coordinates[0]);
		}
		catch (e){}
...
function _mapup_createMarkerClickHandler(map, marker, layer, info, szMode, i) {
	...
	return function() {
		...
		if ( szMode.match(/listitem/) ){
			ixmaps.jsapi.fSidebarClick = true;
		}
		...
		//
		var pageWidth  = parseFloat($("#map").css("width"));
		var mWidth = Math.min(300,pageWidth/3*2);
		var pageHeight = parseFloat($("#map").css("height"));
		var mHeight = Math.min(500,pageHeight/3*2);

		var fDescr = (pageWidth >= 500)?true:false;
		var nClipDescription = (ixmaps.jsapi.mapParam && ixmaps.jsapi.mapParam.all && ixmaps.jsapi.mapParam.all.smallInfoClipDescription)?Number(ixmaps.jsapi.mapParam.all.smallInfoClipDescription):200;
		...
		try	{
			szTitle = ixmaps.jsapi.onInfoWindowTitle(szTitle,info);
		}
		catch (e){}

		// make info content
		//
		var szInfo = "<div class=\"InfoWindowHeader\" style=\"max-width:"+(pageWidth-80)+"px\">"+szTitle+"<\/div>";
		var szDesc = "";
		if ( fDescr || !szTitle.length || (info.properties.description.length < nClipDescription) ){
			szDesc += "<div class=\"InfoWindowBody\" " + (info.properties.description.match(/img/)?"style=\"\"":"") + " >" + info.properties.description + "<\/div>";
		...
		if ( fEmbedAudioInInfo && info.properties.data && info.properties.data.LNK_2 ){
			szDesc += ixmaps.jsapi.getEmbedAudioHTML(ixmaps.jsapi.resolveRelativeUrl(info.properties.data.LNK_2,info.properties.szUrl),250);
		}
		// query user defined info window content	
		//

		try	{
			szDesc = ixmaps.jsapi.onOpenInfoWindow(szDesc,info,"map");
		}
		catch (e){}
		...
		if ( 0 && typeof(InfoBubble) != "undefined" ){
			...
			infoBubble.setContent(szInfo);
			infoBubble.setOnClose(_mapup_onInfoWindowClose);
		}else{
...
MapUp.prototype.isMarkerInFilter = function(layer,marker) {
	if ( ixmaps.jsapi.filterA ){
		for ( f in ixmaps.jsapi.filterA ){
			if ( (f == layer.data.properties.name) && 
				 !( eval('marker.properties.name.match(/'+ixmaps.jsapi.filterA[f]+'/i)')) ){
				return false;
			}
		}
	}
...
MapUp.prototype.isFilter = function() {
	if ( (typeof(ixmaps.jsapi.filterA) != "undefined") && ixmaps.jsapi.filterA.length ){
		return true;
	}
	return false;
};
...
Timeline.prototype.showTimeWindow = function(){
	...
	**/
	...
	var szDate  = "<div style='min-width:100px;color:"+szColor+"'>";
		...
		//szDate += "<a href='javascript:ixmaps.timeline.moveTimeWindow(-1)'> &lt; </a><a href='javascript:ixmaps.timeline.moveTimeWindow(1)'> &gt; </a>";
		//szDate += "&nbsp;";
		//// szDate += "<a href='javascript:ixmaps.timeline.toggleTimeWindow()'> ? </a>";
		//szDate += "</div>";
		szDate += "</div>";
...
//		_mapUp.makeLayer(_mapUp.map,this.layerA[0],this.layerA[0].defaultIcon);
	...
	// inform user
	try{
		ixmaps.jsapi.onChangeTimeWindow(szFlag,nDays);
	}
	catch (e){}
};
...
Timeline.prototype.next = function() {
	...
	if ( $("#showtimelinedate")[0] ){
		...
		$(".post_content").css("visibility","visible"); 
	...
	_mapUp.makeLayer(_mapUp.map,this.layerA[0],this.layerA[0].defaultIcon);
	if ( this.play ){
		ixmaps.timeline = _mapUp.timeline;
		setTimeout("ixmaps.timeline.next();",150);
	}else{
		_mapUp.timeline = _mapUp.savedTimeline;
		_mapUp.timeline.showTimeWindow();
		$(".post_content").css("visibility","hidden"); 
	}
};
...
MapUp.prototype.playTimelineOfLayer = function(szFlag,szName){
	for ( var i=0; i<this.layer.length; i++ ){
		if ( this.layer[i].name == szName ){
