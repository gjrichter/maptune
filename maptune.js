...
maptune.selectFeedDialog = function(flag){
	...
	var szFeedHTMLSrc = MapParam.layerdialog?MapParam.layerdialog.href:'./feeds.html';
...
maptune.selectHelpDialog = function(flag){
	...
	if ( flag && flag === 'hide' ){
		...
		$('#help-content').load('./help.html'+' #content', function(response, status, xhr) {
			...
			ixmaps.touchScroll("help-content");
...
function __addLayerFeedAutoTypeDetection(next){
	...
	ixmaps.message("try:"+__szFeedTypeA[__nFeedType]);

	switch(__szFeedTypeA[__nFeedType]){
		case "kml":
			ixmaps.jsapi.addFeed(feedForm.query.value,{"type":'kml',"format":'xml',"flag":'zoomto',"title":'input'},__addLayerFeedAutoTypeDetectionResult);
			break;
		case "georss":
			ixmaps.jsapi.addFeed(feedForm.query.value,{"type":'GeoRSS',"format":'xml',"flag":'zoomto',"title":'input'},__addLayerFeedAutoTypeDetectionResult);
			break;
		case "xxxgeorss":
			break;
		case "rss-geonames":
		...
		case "rss-metacarta":
			break;
		case "geojson":
			ixmaps.jsapi.addFeed(feedForm.query.value,{"type":'GeoJson',"format":'json',"flag":'zoomto',"title":'input'},__addLayerFeedAutoTypeDetectionResult);
			break;
		case "geojsonR":
...
maptune.addFeedLayer = function(szLayer,opt){
	_TRACE("maptune.addFeedLayer: "+szLayer+" >>>");
	...
	if ( opt && opt.type && (opt.type == "fusiontable") ){
		ixmaps.feed.addFusionTableLayer(szLayer,opt.col,opt.flag,opt.name,opt);
	}else
	if ( opt && opt.type && (opt.type == "fusiontable_query") ){
		ixmaps.feed.addFusionTableQuery(szLayer,opt.col,opt.flag,opt.name,opt);
	}else{
		ixmaps.jsapi.addFeed(szLayer,opt);
	}
...
maptune.addLayer = function(szLayer,szFlag,szFilter,szSource,opt){
	...
	if ( opt.layer ){
		if ( opt.layer.length ){
			for ( l in opt.layer ){
				ixmaps.jsapi.setLayerParam(opt.layer[l]);
			}
		}else{
			ixmaps.jsapi.setLayerParam(opt.layer);
		}
	}
	ixmaps.jsapi.loadData(szLayer,szFilter,"add|"+szFlag,szSource);
	maptune.selectFeedDialog('hide');
};
...
maptune.isFeedLayer = function(szLayer){
	return ixmaps.jsapi.isLayerOfSource(szLayer);
};

// remove layer defined by feed
// ----------------------------------------------------------------
maptune.removeFeedLayer = function(szLayer){
	return ixmaps.jsapi.removeLayerOfSource(szLayer);
};
